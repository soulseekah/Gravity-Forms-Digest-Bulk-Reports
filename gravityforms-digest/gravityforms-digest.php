 ...
 * Text Domain: gravityforms-digest
...
class GFDigestNotifications {
	...
	public static function bootstrap() {
		add_action( 'plugins_loaded', array( __CLASS__, 'early_init' ) );
		add_action( 'gf_digest_send_notifications', array( __CLASS__, 'send_notifications' ), null, 1 );
	...
	public static function reschedule_existing() {
	...
	public static function plugins_loaded() {
		...
		if ( version_compare( GFCommon::$version, '1.7' ) >= 0 ) {
			add_action( 'gform_form_settings_page_digests', array( __CLASS__, 'show_notification_settings' ) );
			add_filter( 'gform_form_settings_menu', array( __CLASS__, 'add_notification_settings_tab' ) );
			return;
		}
		add_filter( 'gform_save_notification_button', array( __CLASS__, 'add_notification_settings' ) );
	}

	/**
	 * Adding a tab in GF 1.7+
	 *
	 * @param array $tabs The current settings tabs.
	 *
	 * @return array The tabs with some additions.
	 */
	public static function add_notification_settings_tab( $tabs ) {
	 ...
	 * This is a GF 1.7+ UI.
	 */
	public static function show_notification_settings() {
		GFFormSettings::page_header();
		echo '<form method="post">';
		echo self::add_notification_settings( '' );
	...
	public static function add_notification_settings( $out ) {
...
if ( defined( 'WP_CONTENT_DIR' ) && ! defined( 'GF_DIGEST_DOING_TESTS' ) )
	GFDigestNotifications::bootstrap();
